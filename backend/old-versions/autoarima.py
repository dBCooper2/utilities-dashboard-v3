...
class AutoARIMAForecast:
    ...
    def evaluate_model(self, test_size:float=0.2) -> Dict[str, Union[float, pd.DataFrame]]:
        ...
        # Generate predictions for test period using existing model
        horizon = len(test_data)
        forecast_df = self.sf.forecast(df=train_data, h=horizon)
        
        # Align forecast dates with test data dates
        forecast_df['ds'] = test_data['ds'].values
        ...
        metrics = {
            ...
            'aic': self.sf.fitted_[0,0].model_['aic'],  # Akaike Information Criterion
            'bic': self.sf.fitted_[0,0].model_['bic'],  # Bayesian Information Criterion
        }
